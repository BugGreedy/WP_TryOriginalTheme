...
## 目次
[第1回 最低限のテーマファイルを作成](#第1回の内容)</br>
[第2回 テンプレートタグを使おう①](#第2回の内容)</br>
[第3回 テンプレートタグを使おう②](#第3回の内容)</br>
[第4回 ループを使って一覧を表示させる](#第4回の内容)</br>
[第5回 テンプレート改装で、投稿ページと固定ページを作ろう](#第5回の内容)</br>
[第6回 functions.phpでサムネイル画像を表示しよう](#第6回の内容)</br>
[第7回 本文に入力欄を追加できるカスタムフィールドを使おう](#第7回の内容)</br>
[第8回 カスタム投稿(ポスト)タイプで投稿欄を増やそう](#第8回の内容)</br>
[第9回 カスタム投稿(ポスト)タイプのテンプレートファイルを作ろう](#第9回の内容)</br>
[第10回 カスタム分類（タクソノミー）で、分類項目を追加しよう](#第10回の内容)</br>
[第11回 ヘッダー・フッターとページテンプレートについて、その他追加情報](#第11回の内容)</br>
...
## 第11回の内容
### ヘッダー・フッターとページテンプレートについて、その他追加情報
...
3. その他WordPressではどんなパーツでも切り出して共通部分として使用する事ができる。
   ...
   ```
   先程と区別するため"includes"の中に`footer02`という名前のファイルを作成し、切り出した部分を貼り付ける。</br>
   </br>
4. その他、見た目について</br>
   現状、ナビゲーションメニュー(トップに表示されるHOMEやABOUTなど)がWordPressの管理メニューバーにかぶっているため、メニューバーの方の位置を変える。</br>
   WordPressメニューのプラグイン→新規追加で"bottom admin bar"などで検索し、メニューバーをページの下部に表示できるプラグインを有効化する。</br>
   </br>
5. 特別な固定ページの作成
   例えばキャンペーンやランディングページなど通もとのブログとは別の作りをした固定ページを作りたいとき、こちらカスタム要素と同様にそのページ用のテンプレートを作成して優先して使用させる事が可能。</br>
   仮に**特別な固定ページ**という名前の固定ページを作成する。</br>
   通常固定ページは`page.php`というテンプレートが使われている。WordPressの[テンプレート階層図](https://unofficialtokyo.com/wordpress-template-hierarchy/)によると、この次に優先されるのは`page-$id.php`というテンプレートファイル名である。`$id`とは固定ページにごとに割り振られる番号で、その固定ページのエディット時のURLのpost=""に表示されるもの。例：`http://localhost:8888/mamp_wordpress_handson/wp-admin/post.php?post=2141&action=edit/`の場合だと**2141**が$idである。</br>
   この場合のテンプレートファイル名は`page-2141.php`である。</br>
   </br>
   ただし、この場合はその次に優先される`page-$slug.php`の方が管理しやすい。他の投稿と同じようにエディット画面のパーマリンクより独自のスラッグを設定して割り当てる事ができる。</br>
   `page.php`を複製して特別な固定ページ用のテンプレートファイルを作成する。今回はスラッグを`special`としたのでこの場合のテンプレートファイル名は`page-special.php`となる。</br>
   </br>
   また、この場合だと同様のページを作るごとに`page-special.php`を複製して各テンプレートファイルを作っていかないといけないので手間がかかる。</br>
   そこで特別なページ用のテンプレートを作成して都度そのテンプレートを使用できるようにする。</br>
   先程作った`page-special.php`の名前を任意に変更する。ただし他のテンプレート階層図にある標準ページテンプレートの名称とかぶるのは避ける必要がある。そのため階層上ありえない名前を設定する必要がある。方法として`page-template-"".php`などとするとかぶらなくてよい。今回は仮に`page-template-lp.php`とする。</br>
   </br>
   ここでこの特別なカスタムテンプレートにはあるコメントを記述する必要がある。
   作成した`page-template-lp.php`の最上部を下記のように編集する。</br>
   この記述をWordPress側が読み取るので正確に記述する必要がある。
   ```php
   <?php
   /*
   ...
   */
   ...
   以下は略
   ```
   このような記述があるファイルが存在する場合のみ、固定ページ投稿エディタ画面の右側のメニュー内に**ページ属性**という項目が表示される。この中から記述内で名付けた**LP用レイアウト**を選択することによりこのページにカスタムテンプレートを適用することができる。</br>
